...
// ==========================================================================
namespace {
    const char* nome_modelo = "CNN_32x32_v1";
...
bool inicializar_tflm() {
    ...
    printf("Arena REAL utilizada pela autoral: %d bytes\n", tflInterpreter->arena_used_bytes());
...
{
    memcpy(tflInputTensor->data.int8, input_data, kImageSize);
    ...
    int num_classes = tflOutputTensor->dims->data[1]; // deve ser 3

    int8_t* out_q = tflOutputTensor->data.int8;

    int best_idx = 0;
    int8_t best_val = out_q[0];
    ...
    // (opcional) dequantizar só para debug
    float prob = (best_val - tfOutputZeroPoint) * tfOutputScale;

    printf("Classe=%d  score=%.3f\n", best_idx, prob);
                ...
                const uint8_t* data, int data_len)
{
    ...
    {
        // tamanho esperado
        int esperado = sizeof(int64_t) + kImageSize;

        if (data_len != esperado)
        {
            printf("Erro tamanho: %d esperado: %d\n", data_len, esperado);
            return;
        }

        printf("Imagem + tempo recebidos\n");

        // ================================
        // extrair tempo
        // ================================
        int64_t timestamp_envio;

        memcpy(&timestamp_envio, data, sizeof(int64_t));

        // ================================
        // extrair imagem
        // ================================
        const int8_t* imagem =
            (const int8_t*)(data + sizeof(int64_t));

        // ================================
        // timestamp chegada ESP
        // ================================
        int64_t timestamp_recebido =
            esp_timer_get_time();

        int64_t latencia =
            timestamp_recebido - timestamp_envio;

        printf("Latencia cam->esp: %.3f ms\n",
               latencia / 1000.0);
        ...
        int64_t tempo_inferencia =
            fim_inf - inicio_inf;

        char json_resposta[128];

        sprintf(json_resposta,
        "{"
        "\"resultado\":%d,"
        "\"timestamp_envio\":%.6f"
        "}",
        resultado,
        inicio_inf / 1000000.0
        );
        ...
        printf("Tempo inferencia: %.3f ms\n",
               tempo_inferencia / 1000.0);

        // ================================
        // enviar métricas via MQTT
        // ================================
        char json[512];
        ...
        "\"modelo\":\"%s\","
        "\"timestamp_envio\":%.6f,"
        "\"timestamp_recebido\":%.6f,"
        "\"latencia_ms\":%.3f,"
        "\"tempo_inferencia_ms\":%.3f,"
        "\"resultado\":%d"
        "}",
        nome_modelo,
        timestamp_envio / 1000000.0,
        timestamp_recebido / 1000000.0,
        latencia / 1000.0,
        tempo_inferencia / 1000.0,
        resultado);
        ...
        printf("Metricas enviadas via MQTT\n");
    }
